...
function DisplayWorks() {
  ...
  const items = [
    ...
    {type: 'A', status: 'published', title: 'Item A2',view:'1',save:'2',earn:'3'},
    {type: 'A', status: 'draft', title: 'Item A3'},
    {type: 'A', status: 'published', title: 'Item A4',view:'1',save:'2',earn:'3'},
    {type: 'A', status: 'published', title: 'Item A5',view:'1',save:'2',earn:'3'},
              ...
              {publishedItems.map((item, index) => (
                <div className="work_square" key={index}>
                  ...
                  <div className="work_content">
                    <div className={`item item-${item.type}}`} >
                      <h3>{item.title}</h3>
                      <div style={{width:'100%',display:'flex',justifyContent:'space-around',flexDirection:'row',alignItems:'space-around',marginBottom:'5%',fontSize:'10px'}}>
                        <div style={{display:'flex',justifyContent:'flex-start',flexDirection:'column',alignItems:'center'}}>
                          觀看數<div>{item.view}</div></div>
                          <div style={{display:'flex',justifyContent:'flex-start',flexDirection:'column',alignItems:'center'}}>
                          收藏數<div>{item.save}</div></div>
                          <div style={{display:'flex',justifyContent:'flex-start',flexDirection:'column',alignItems:'center'}}>
                          收益量<div>{item.earn}</div></div>
                      </div>
