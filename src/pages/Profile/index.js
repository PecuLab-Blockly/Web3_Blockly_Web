  ...
  const handlePicUpload = (event) => {
    if (event.target.files && event.target.files[0]) {
      ...
      reader.onload = (e) => {
        ...
        // 將背景顏色設為透明
        document.querySelector('.profile-pic').style.backgroundColor = 'transparent';
      };
      ...
      <div className="container">
        <div className="column one">
          ...
          <div className="column_one_left">
            <div className="profile-pic" style={{ backgroundImage: profilePic ? `url(${profilePic})` : '', backgroundColor: profilePic ? 'transparent' : '#ccc' }}>
              {!profilePic && (
                <>
            ...
            {!editingName && (
              <div style={{flexDirection:'row',display:'flex'}}>
                ...
                <FontAwesomeIcon icon={faEdit} onClick={() => setEditingName(true)} />
              </div>
            )}
            {editingName && (
              <div style={{flexDirection:'row',display:'flex'}}>
                <input type="text" value={name} onChange={(e) => setName(e.target.value)} />
                <button style={{height:'60%'}} onClick={() => setEditingName(false)}>儲存</button>
              </div>
            )}
          </div>
